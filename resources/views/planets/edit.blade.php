        ...
        <form action="{{ route('planets.update', $planet->id) }}" method="post">
            ...
            <div class="block mt-4">
                <label for="state" class="inline-flex items-center">
                    <input id="state" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="state" @if(old('state', $planet->state)) checked @endif>
                    <span class="ml-2 text-sm text-gray-600">{{ __('Planet done') }}</span>
                </label>
            </div>
